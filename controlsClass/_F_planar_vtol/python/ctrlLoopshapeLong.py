...
import loopShaping_vtol_long as L

class ctrlLoopshapeLong:
    ...
    def update(self, hRef, y):
        h = y[1][0]

        # prefilter the reference
        h_r = self.prefilter.update(hRef)

         # filtered error signal
        errorH = h_r - h

        # update controller
        force_tilde = self.control.update(errorH)
        force_fl = P.g * (2*P.mr + P.mc)
        force = saturate(force_fl + force_tilde, 2*P.fmax)
