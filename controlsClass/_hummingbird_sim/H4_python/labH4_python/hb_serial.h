...
#ifndef HB_SERIAL_H
#define HB_SERIAL_H         // An if statement for this code, if HB_SERIAL_H is not defined, then this code runs and defines HB_SERIAL_H

#include <stdbool.h>        // Include boolean variable types
#include <stdint.h>         // Include integer variable types
#include <string.h>         // Include string variable types and string manipulation functions
...
typedef enum
{
  ...
  HB_SERIAL_MSG_ZERO,
  HB_SERIAL_NUM_MSGS
} hb_msg_type_t;            // This defines an enumeration, which is a variable 
                            // type that can be assigned to another name, and 
                            // that enumeration has several integer with the names 
                            // inside
...
typedef struct
{
  ...
  float pitch;
  float roll;
} hb_serial_encoder_msg_t;  // This defines a structure, which is similar to an 
                            // enum in that it is a user defined type, but it differs
                            // in that it can contain varied data types while an enum
                            // can only have integers
...
static constexpr float HB_SERIAL_PAYLOAD_LEN[] = {
  ...
  sizeof(bool),
  sizeof(bool)
};                                // Calculate the total memory space required to transmitting the following data 
